...
class CategoryGridFrame(QWidget):
    ...
    def save_as_variation(self):
        """Guarda los valores actuales como una variación"""
        # Recopilar valores actuales
        variation_data = {}
        changes = {}
        
        for card in self.cards:
            if hasattr(card, 'category_name') and hasattr(card, 'get_value'):
                value = card.get_value().strip()
                if value:
                    # Detectar cambios comparando con valores por defecto
                    old_value = getattr(card, 'default_value', '')
                    if value != old_value:
                        changes[card.category_name] = {
                            'old': old_value,
                            'new': value
                        }
                    variation_data[card.category_name] = value
    ...
    class SaveTypeDialog(QDialog):
        ...
        def setup_ui(self):
            ...
            # Título
            title = QLabel("¿Cómo deseas guardar los datos?")
            ...
            self.new_character_btn.setFixedHeight(35)
            self.new_character_btn.clicked.connect(self.select_new_character)
            button_layout.addWidget(self.new_character_btn)
            
            self.variation_btn = QPushButton("Variación")
            self.variation_btn.setFixedHeight(35)
            self.variation_btn.clicked.connect(self.select_variation)
            button_layout.addWidget(self.variation_btn)
        ...
        def select_new_character(self):
            self.save_type = "new_character"
            self.accept()
        
        def select_variation(self):
            self.save_type = "variation"
            self.accept()
